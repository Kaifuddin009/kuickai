      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3'>
          ...
          {articleLength.map((item, index)=>(
            ...
            className={`text-sm p-4 py-1 border rounded-full cursor-pointer ${selectedLenght.text === item.text ? 'bg-blue-50 text-blue-700':'text-gray-500 border-gray-300'}`}
        ...
        <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
          ...
          <p className='text-xl'>Enter a topic & click "Generate Article" to get started</p>
            </div>
          </div>
        </div>
    </div>
